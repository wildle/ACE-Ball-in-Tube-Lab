classdef Tc3AutomationInterface < handle
...
%   Author: Beckhoff Automation GmbH & Co. KG
    ...
    methods
        ...
        %   set porperty: AssembliesLoaded = TRUE/FALSE
            try                
                ...
                % load assemblies
                assemblyPath = 'C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\Common7\IDE\PublicAssemblies\envdte.dll';
                assemblyPath80 = 'C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\Common7\IDE\PublicAssemblies\envdte80.dll';
                % COM lib for Visual Studio Core Automation - for VS specific commands
                NET.addAssembly(assemblyPath);   % doc -> https://docs.microsoft.com/en-us/dotnet/api/envdte
                NET.addAssembly(assemblyPath80); % doc -> https://docs.microsoft.com/en-us/dotnet/api/envdte80
        ...
        function TcComObject = ChangeTcCOM(this, Modelname)
        % CreateTcCOM  creates a new instance of a TcCOM
        %
        %   TcComObject = CreateTcCOM(Modelname)
        %   Instanciates the TcCOM with the specified name (Modelname).
        ...
        %   >Beckhoff Infosys</a>   
            ...
            % (2) Create a TcCom Object
             % doc -> https://infosys.beckhoff.com/content/1033/tc3_automationinterface/27021598006995083.html?id=5908679173508422650
            parentTreeItem = this.sysManager.LookupTreeItem('TIRC^TcCOM Objects'); % get a pointer to a tree item; shortcut TIRC = real-time configuration, see also link above
            parentTreeItem.DeleteChild(['Ctrl_only_Obj1 (', Modelname,')']);
            ...
            TaskOID = cell(1,numel(cycleTime));
            for i = 1:numel(cycleTime)
                Tasks.DeleteChild(['TaskFor' Modelname '_' num2str(i)])
                Task = Tasks.CreateChild(['TaskFor' Modelname '_' num2str(i)],1,[],[]);
                Task.ConsumeXml(['<TreeItem><TaskDef><Priority>' 1 '</Priority></TaskDef></TreeItem>']);
                Task.ConsumeXml(['<TreeItem><TaskDef><CycleTime>' cycleTime{i}(1:length(cycleTime{i})-2) '</CycleTime></TaskDef></TreeItem>']); % Has to be scaled to base tick so delete last two zeroes: cycleTime(1:length(cycleTime)-2)
            ...
            % Position Link
            this.LinkVariables([char(TcComObject.PathName),'^Ctrl_only_U^actpos'], 'TIID^Device 2 (EtherCAT)^Box 6 (BallInTubeControl)^Inputs^actPosition');
            % RPM Link
            this.LinkVariables([char(TcComObject.PathName),'^Ctrl_only_U^rpm'], 'TIID^Device 2 (EtherCAT)^Box 6 (BallInTubeControl)^Inputs^actRPM_FAN');
            % Duty Cycle Link
            this.LinkVariables([char(TcComObject.PathName),'^Ctrl_only_Y^dutyCyc'], 'TIID^Device 2 (EtherCAT)^Box 6 (BallInTubeControl)^Outputs^DutyCyclePWM_FAN');
            % State Link
            this.LinkVariables([char(TcComObject.PathName),'^Ctrl_only_Y^ctrl'], 'TIID^Device 2 (EtherCAT)^Box 6 (BallInTubeControl)^Outputs^Ctrl');
        end
        ...
        %   >Beckhoff Infosys</a>
            ...
            if (this.ReadTwinCatState(AmsNetId) ~= TwinCAT.Ads.AdsState.Config)
                ...
                for ii = 0:1:(xmlDeviceList.Count-1) % devices start with 0
                    ...
                    % ignore devices that are no EtherCAT masters
                    if(itemSubType == 111)
        ...
        function SwitchToConfig(this,AmsNetId)
        ...
        function closeVsInstance(this)
            this.VsDTE.Quit();
        end

        function TcComObject = ReloadTmcFile(this, tmcFilePath, tcComObjectName)

             tcComModule = this.sysManager.LookupTreeItem('TIRC^TcCOM Objects');
             if isempty(tcComModule)
                error('TcCOM-Objekt nicht gefunden');
            end
             %tcComModule.PlcAutoConfig = false;
             tcComModule.DeleteChild(tcComObjectName); % Entfernt alle bisherigen Konfigurationen
             TcComObject = tcComModule.CreateChild(tcComObjectName, tmcFilePath, 'Module');
        % 
        %     % ReloadTmcFile reloads the code-generated TMC file into the TcCOM object.
        %     %
        %     % tmcFilePath - Path to the TMC file to be loaded
        % 
        %     if strcmpi(AmsNetId, 'Local')
        %         AmsNetId = char(TwinCAT.Ads.AmsNetId.Local.ToString()); 
        %     end
        % 
        %     % Scann Folder for Newest Modul, ...
        % 
        % 
        % 
        %     if ~exist(tmcFilePath, 'file')
        %         error('TMC file does not exist: %s', tmcFilePath);
        %     end
        % 
        %     % Set target system
        %     this.sysManager.SetTargetNetId(AmsNetId);
        %     pause(2); % wait for target system to be set
        % 
        %     xTmcFile = System.Xml.XmlDocument;
        %     % Reload TMC file into TcCOM object
        %     xTmcFile.Reload(tmcFilePath); %?
        % 
        % 
         end
    ...
    methods (Static)        
        function vsVersions = GetInstalledVisualStudiosI
    ...
    % GetInstalledVisualStudios  gets all installed Visual Studio versions
    %
    %   vsVersions = GetInstalledVisualStudios
    %   Static Method. Looks up all installed Visual Studio versions.
    %   Returns an array of numbers (vsVersions)
    ...
    % List of Visual Studio versions to check
    versionsToCheck = [10, 11, 12, 14, 15, 16];

    % Registry keys for different Visual Studio installation paths
    regKeys = {
        'SOFTWARE\Microsoft\VisualStudio\', ...
        'SOFTWARE\Wow6432Node\Microsoft\VisualStudio\', ...
        'SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\X64' % Add key for VS 2019
    };

    % Iterate over each version to check
    for vsVer = versionsToCheck
        % Iterate over each registry key
        for i = 1:numel(regKeys)
            if vsVer == 16
                % Special case for Visual Studio 2019
                keyPath = regKeys{3};
            else
                % Standard case for other versions
                keyPath = [regKeys{i} num2str(vsVer) '.0'];
            end

            try
                % Query the registry for the 'InstallDir' value
                installDir = winqueryreg('HKEY_LOCAL_MACHINE', keyPath, 'Version');
                
                % If the InstallDir is successfully retrieved, add version to the list
                if ~isempty(installDir)
                    vsVersions = vsVer;  %#ok<AGROW>
                    break;  % Exit the loop if the version is found
                end
            catch ME
                % Handle any errors encountered during registry query
                warning('Error querying registry key "%s": %s', keyPath, ME.message);
            end
        end
    end
end

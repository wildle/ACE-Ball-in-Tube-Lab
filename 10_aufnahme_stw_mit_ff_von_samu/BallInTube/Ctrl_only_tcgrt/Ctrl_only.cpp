...
void Ctrl_only_step(RT_MODEL_Ctrl_only_T *const Ctrl_only_M)
{
  ...
  t = (Ctrl_only_B->Delay + 1.0) * 0.005;
  hE_traj = std::floor(t / Ctrl_only_P.T_rep_Value);
  if (std::isnan(hE_traj) || std::isinf(hE_traj)) {
    xp_ref = (rtNaN);
  } else if (hE_traj == 0.0) {
    xp_ref = 0.0;
  } else {
    xp_ref = std::fmod(hE_traj, 2.0);
    if (xp_ref == 0.0) {
      xp_ref = 0.0;
    } else if (hE_traj < 0.0) {
      xp_ref += 2.0;
  ...
  if (xp_ref != 0.0) {
    h0_traj = Ctrl_only_P.hE_Value;
    hE_traj = Ctrl_only_P.h0_Value;
  } else {
    h0_traj = Ctrl_only_P.h0_Value;
    hE_traj = Ctrl_only_P.hE_Value;
  }

  xp_ref = 0.0;
  xpp_ref = 0.0;
  xppp_ref = 0.0;
  tau = rt_remd_snf(t, Ctrl_only_P.T_rep_Value);
  if (!(tau > Ctrl_only_P.T_Value)) {
    hE_traj -= h0_traj;
    xppp_ref = hE_traj * 10.0 / rt_powd_snf(Ctrl_only_P.T_Value, 3.0);
    a4 = hE_traj * -15.0 / rt_powd_snf(Ctrl_only_P.T_Value, 4.0);
    a5 = hE_traj * 6.0 / rt_powd_snf(Ctrl_only_P.T_Value, 5.0);
    xpp_ref = rt_powd_snf(tau, 3.0);
    xp_ref = rt_powd_snf(tau, 4.0);
    hE_traj_tmp = tau * tau;
    hE_traj = ((((0.0 * tau + h0_traj) + hE_traj_tmp * 0.0) + xppp_ref * xpp_ref)
               + a4 * xp_ref) + a5 * rt_powd_snf(tau, 5.0);
    xp_ref = (3.0 * xppp_ref * hE_traj_tmp + 4.0 * a4 * xpp_ref) + 5.0 * a5 *
      xp_ref;
    xpp_ref = (6.0 * xppp_ref * tau + 12.0 * a4 * hE_traj_tmp) + 20.0 * a5 *
      xpp_ref;
    xppp_ref = (24.0 * a4 * tau + 6.0 * xppp_ref) + 60.0 * a5 * hE_traj_tmp;
  }

  tau = std::sqrt(xpp_ref + 9.81);
  h0_traj = (0.209195 * xp_ref + tau) / 0.000745736;
  tau = (xppp_ref / (2.0 * tau) + 0.209195 * xpp_ref) / 0.000745736;
  Ctrl_only_B->u_ref_m = (1.255 * tau + h0_traj) / 96.295 - 34.234;
  Ctrl_only_B->h_ref = hE_traj;
  Ctrl_only_B->x_out[0] = hE_traj;
  Ctrl_only_B->x_out[1] = xp_ref;
  Ctrl_only_B->x_out[2] = xpp_ref;
  Ctrl_only_B->x_out[3] = h0_traj;
  Ctrl_only_B->x_out[4] = tau;
  Ctrl_only_B->t = t;
  ...
  if (Ctrl_only_P.disableu_ref_CurrentSetting == 1) {
    /* ManualSwitch: '<S4>/disable u_ref' */
    Ctrl_only_B->u_ref = Ctrl_only_B->u_ref_m;
  } else {
    /* ManualSwitch: '<S4>/disable u_ref' incorporates:
     *  Constant: '<S4>/Constant2'
     */
    Ctrl_only_B->u_ref = Ctrl_only_P.Constant2_Value;
  }
  ...
  if (Ctrl_only_P.ControlvsConstant_CurrentSettin == 1) {
    ...
    /* Sum: '<S4>/Sum' */
    Ctrl_only_B->Sum = Ctrl_only_B->u_ref + Ctrl_only_B->PIDvsFF;
  ...
  t = std::floor(Ctrl_only_B->procent2ticks);
  if (std::isnan(t) || std::isinf(t)) {
    t = 0.0;
  } else {
    t = std::fmod(t, 65536.0);
  }

  /* DataTypeConversion: '<S2>/Data Type Conversion2' */
  Ctrl_only_B->DataTypeConversion2 = static_cast<uint16_T>(t < 0.0 ?
    static_cast<int32_T>(static_cast<uint16_T>(-static_cast<int16_T>(
    static_cast<uint16_T>(-t)))) : static_cast<int32_T>(static_cast<uint16_T>(t)));
...
#include "Ctrl_only_reg_dt.h"
...

  {
    ...
    /* External mode info */
    Ctrl_only_M->Sizes.checksums[0] = (2624189851U);
    Ctrl_only_M->Sizes.checksums[1] = (122118707U);
    Ctrl_only_M->Sizes.checksums[2] = (3412832266U);
    Ctrl_only_M->Sizes.checksums[3] = (1444555790U);
